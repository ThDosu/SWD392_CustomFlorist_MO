...
export default function MainLayout() {
    const navigation = useNavigation();
    const [selectedTab, setSelectedTab] = useState("Home"); // ✅ Quản lý tab
        ...
        <View style={{ flex: 1 }}>
            ...
            >
                {tabs.map((item, index) => (
                    <TouchableOpacity
                        key={index}
                        onPress={() => setSelectedTab(item.screen)} // ✅ Đổi tab
                    >
                        <Image
                            source={item.icon}
                            style={{
                                height: 24,
                                width: "100%",
                                tintColor: selectedTab === item.screen ? COLORS.green : COLORS.gray2,
                                marginBottom: 10,
                            }}
                            resizeMode="contain"
                        />
                        <Text
                            style={{
                                ...
                                fontSize: 14,
                                lineHeight: 14 * 1,
                                color: selectedTab === item.screen ? COLORS.green : COLORS.gray2,
                            }}
                        >
                            {item.screen}
                        </Text>
